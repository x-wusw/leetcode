...
/*
        ...
        如果鸡蛋不碎，那么状态变成 (K,N−X)，即我们鸡蛋的数目不变，但答案只可能在上方的 N-X,N−X 层楼了。也就是说,
        我们把原问题缩小成了一个规模为(K,N−X) 的子问题.
        如果鸡蛋碎了，那么状态变成 (K−1,X−1)，即我们少了一个鸡蛋，但我们知道答案只可能在第 X 楼下方的 X-1X−1 层楼中了。
        也就是说，我们把原问题缩小成了一个规模为(K−1,X−1) 的子问题。
        这样一来，我们定义 dp(K,N) 为在状态(K,N) 下最少需要的步数。根据以上分析我们可以列出状态转移方程：
        dp(K, N) = 1+ min(max(dp(K−1,X−1)),max(dp(K,N−X)))

        第一个鸡蛋扔在的楼层数 X。由于我们并不知道真正的 F 值，因此我们必须保证 鸡蛋碎了之后接下来需要的步数和鸡蛋没碎之后接下来
        需要的步数二者的最大值最小，这样就保证了在最坏情况下（也就是无论F的值如何）dp(K,N) 的值最小。如果能理解这
        一点，也就能理解上面的状态转移方程，即最小化max(dp(K−1,X−1),dp(K,N−X))。

        T1 = dp(K−1,X−1)  T2 = dp(K,N−X)   T1随x增大而增大，T2随x增大而减小
        找到最大的满足T1(x)<T2(x)以及最小的满足T1(x)>=T2(x)的x ，就是离这两个函数（想象中的）交点左右两侧最近的整数。
