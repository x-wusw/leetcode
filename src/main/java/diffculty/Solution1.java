...
public class Solution1 {
     ...
     *
     *   要确保这两个条件满足：
     *   1.i + j = m - i + n - j(或m - i + n - j + 1)  如果n >= m。只需要使i = 0 ~ m，j = (m+n+1)/2-i =====>
     *   该条件在m+n为奇数/偶数时，该推理都成立
        ...
        while (iMin <= iMax) {
            ...
            if (i < iMax && B[j - 1] > A[i]) {
                ...
                if (i == 0) {//A分成的leftA(空集) 和 rightA(A的全部)  所以leftPart = leftA(空集) + leftB,故maxLeft = B[j-1]。
                    maxLeft = B[j - 1];
                } else if (j == 0) { //B分成的leftB(空集) 和 rightB(B的全部)  所以leftPart = leftA + leftB(空集),
                    // 故maxLeft = A[i-1]。
                    maxLeft = A[i - 1];
                ...
                //偶数
                int minRight;
                if (i == m) {//A分成的leftA(A的全部) 和 rightA(空集)  所以rightPart = rightA(空集) + rightB,
                    // 故minRight = B[j]。
                    minRight = B[j];
                } else if (j == n) {//B分成的leftB(B的全部) 和 rightB(空集)  所以rightPart = rightA + rightB(空集),
                    // 故minRight = A[i]。
                    minRight = A[i];
                } else {//排除上述两种特殊情况，正常比较
    ...
    public static void main(String[] args) {
        ...
        System.out.println("中位数："+s.findMedianSortedArrays(A,B));
    }
}
